...
object ShopRotation {
    ...
    fun <T> rotateCatalog(
        ...
        val nextDay = day + CYCLE_DAYS_SHORT
        val previewPool = rotating.filter {
            val id = idOf(it)
            !isActiveInRotation(id, day) && isActiveInRotation(id, nextDay)
        }.sortedWith(compareBy({ priceOf(it) }, { idOf(it) }))
        ...
        val preview = buildList {
            ...
            if (size < previewCount) {
                for (item in rotating.sortedByDescending { priceOf(it) }) {
                    if (size >= previewCount) break
                    if (item !in mixed && item !in this) add(item)
                }
            }
        }.take(previewCount)
