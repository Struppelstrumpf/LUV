...
object ShopCatalog {
    ...
    val EMOJIS: List<ShopEmoji> = listOf(
        ...
        ShopEmoji("🏆", 14), ShopEmoji("👑", 16), ShopEmoji("💎", 18),
        ...
        ShopEmoji("🫀", 1500), ShopEmoji("🎭", 1200), ShopEmoji("🏆", 1100),
    ...
    val STICKERS: List<ShopEmoji> = listOf(
        ...
        ShopEmoji("🏆", 900), ShopEmoji("💍", 1100), ShopEmoji("🐉", 1200),
...
object LiveShopCatalog {
    ...
    fun displayLabel(kind: String, itemId: String): String? {
        ...
        displayLabels["$k:$id"]?.trim()?.takeIf { it.isNotEmpty() }?.let { return it }
        // Emoji-Varianten (z. B. mit/ohne VS16) abfangen
        val bare = id.replace("\uFE0F", "")
        if (bare == id) return null
        displayLabels["$k:$bare"]?.trim()?.takeIf { it.isNotEmpty() }?.let { return it }
        return displayLabels.entries.firstOrNull { (key, value) ->
            val parts = key.split(":", limit = 2)
            parts.size == 2 &&
                parts[0].equals(k, ignoreCase = true) &&
                parts[1].replace("\uFE0F", "") == bare &&
                value.isNotBlank()
        }?.value?.trim()
    }
